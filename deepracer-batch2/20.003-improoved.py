...
# This model will work well if trainned more
...
def reward_function(params):
    ...
    angle_list = [(0 if((angle_val - 2)<0) else (angle_val - 2)) for angle_val in angle_list]
...
#        if (steps % 100) == 0 and progress > ((steps * min_dist_point) / TOTAL_NUM_STEPS) * 100:
#            reward_factor = 1.2
        
#        # Penalize for excessive steering to prevent zig-zag behavior
#        if abs(steering_angle) > STEERING_THRESHOLD:
#            desired_steering_reward = desired_steering_reward * 0.95
         
        if(current_angle<1 and desired_steering_reward > 0.90):
#            reward_factor = 1 + progress
